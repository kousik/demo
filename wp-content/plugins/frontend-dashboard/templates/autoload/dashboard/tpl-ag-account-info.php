...
<div class="agent-view" <?=$style?>>
    ...
    $edit_link = site_url("/dashboard/")."?menu_type=user&menu_slug=account_info&&fed_nonce=". wp_create_nonce( 'fed_nonce' )."&display=agedit&rid=".encrypt_decrypt('encrypt',$user->ID);
    ?>
    <div class="row">
...
<div class="agents-edit" <?=$edit_style?>>
    ...
    <form class="etf-hub-form-agent form-horizontal" id="etf-hub-form-agent" action="<?php echo site_url( 'wp-load.php' );?>" name="etf-community-form" enctype="multipart/form-data" method="post">
        ...
        <?php
        $link = site_url("/dashboard/")."?menu_type=user&menu_slug=account_info&fed_nonce=" . wp_create_nonce( 'fed_nonce' );
        ?>
