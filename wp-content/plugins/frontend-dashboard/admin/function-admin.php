...
function fed_get_input_details( $attr ) {
	...
	if($values['name'] == "state" || $values['name'] == 'city'){
        $values['input_type'] = 'select';
        if($values['name'] == "state") {
            $values['class'] = "chosen-select ".$values['class'] . " js-profile-state";
        }

        if($values['name'] == 'city') {
            $values['class'] = "chosen-select ". $values['class'] . " js-profile-city";
        }
    }
            ...
            if($values['name'] == "state" || $values['name'] == 'city'){
                ...
                if($values['name'] == "state") {
                    ...
                    if ( is_user_logged_in() ) {
                        ...
                        $def_state = get_user_meta($current_user->ID, 'state', true);
                    }
                    $options = fdb_get_state();
                    $input .= '<select ' . $values['disabled'] . ' ' . $values['id'] . ' ' . $values['extra'] . ' name="' . $values['name'] . '"  class="' . $values['class'] . '" data-placeholder="Choose a State..." tabindex="2">';
                    $input .= '<option value="" >Select State....</option>';

                    foreach ($options as $key => $label) {
                        $input .= '<option
						value="' . esc_attr($label->name) . '" ' . selected($def_state, $label->name, false).'>' . $label->name . '</option>';
                    }
                    $input .= '</select>';
                }

                if($values['name'] == 'city') {
                    $input .= '<select ' . $values['disabled'] . ' ' . $values['id'] . ' ' . $values['extra'] . ' name="' . $values['name'] . '"  class="' . $values['class'] . '" data-placeholder="Choose a City..." tabindex="2">';
                    $def_city = '';
                    if ( is_user_logged_in() ) {
                        $current_user = wp_get_current_user();
                        $def_city = get_user_meta($current_user->ID, 'city', true);
                        if($def_city) {
                            $def_state = get_user_meta($current_user->ID, 'state', true);
                            $cities = fdb_get_city($def_state);
                            if($cities){
                                foreach ($cities as $ckey => $cobj) {
                                    $input .= '<option value="'.$cobj->name.'" ' . selected($def_city, $cobj->name, false).'>'.$cobj->name.'</option>';
                                }
                            } else {
                                $input .= '<option value="Not applicable" >Not applicable..</option>';
                            }
                        }
                    }
