...
function fed_fetch_user_profile_required_by_menu( $menu = 'profile' ) {
	...
	$result = $wpdb->get_results( "SELECT * FROM $table_name WHERE (menu LIKE '{$menu}' AND is_required LIKE 'true' AND show_user_profile = 'Enable') ", ARRAY_A );
	if ( count( $result ) <= 0 ) {
		return array();
	}
	if ( $result == null ) {
...
function fed_fetch_user_profile_by_menu_slug($menu_slug = '') {
	...
	$results = $wpdb->get_results( "SELECT * FROM $table_name WHERE show_dashboard LIKE 'Enable' AND menu LIKE '{$menu_slug}'  ORDER BY `input_order` ASC", ARRAY_A );
