...
//Filters
add_filter('cron_schedules', 'epictions_filter_cron_schedules',1);
...
/**
* Add an "every 5 minutes" cron item
* @param Array The cron schedule array
* @return Array The updated cron schedule array
*/
function epictions_filter_cron_schedules($param) {
    $param['every_5_minutes'] = array('interval'=>300, 'display'=>'Once every five minutes');
    //$param['every_1_seconds'] = array('interval'=>1, 'display'=>'Once every one Seconds');
	$param['every_2_hours'] = array('interval'=>7200, 'display'=>'Once every two Hours');
	$param['every_24_hours'] = array('interval'=>86400, 'display'=>'Once every twelve Hours');
	$param['every_one_day'] = array('interval'=>86400, 'display'=>'Once every day');
	$param['every_one_minute'] = array('interval'=>60, 'display'=>'Once every minute');
    $param['every_week_total_user'] = array('interval'=>604800, 'display'=>'Once every week');
    return $param;
}
 ...
 * @return array
 */
function get_epic_trial_start_end_date($date, $day = 0){
	$time_end = "23:59:59";
	$time_start = "00:00:01";


	$data = [];
	$starts = date('Y-m-d', strtotime($date. ' + '.$day.' days'));
	$starts_at = $starts." ".$time_start;


	$d = $day-1;
	$ends = date('Y-m-d', strtotime($date. ' + '.$d.' days'));
	$ends_at = $ends." ".$time_end;

	if($day == 0):
		$ends_at = date('Y-m-d', strtotime($starts. ' - 1 days'));
		$ends_at = $ends_at." ".$time_end;
	endif;
	$data['start_at'] = $starts_at;
	$data['end_at'] = $ends_at;
...
function epic_content_global_pagination($page, $data_arr, $permalink, $jump_to = false){
    ...
    <ul><?php
    if( $data_arr['paged'] > 1){ ?>
        <li class="previous"><a href="<?php echo epic_data_escape($permalink."{$join}pg=".($page-1), "url") ?>"><i class="icon-arrow-left"></i> Previous</a></li><?php
    }
    ...
    <li <?php if( $page == $i ) echo 'class="active"' ?>><a href="<?php echo epic_data_escape($permalink."{$join}pg=".intval($i), "url") ?>"><?php echo intval($i) ?></a></li><?php
    }
    if( $data_arr['max_page'] > $data_arr['paged'] ){ ?>
        <li class="next"><a href="<?php echo epic_data_escape($permalink."{$join}pg=".($page+1), "url") ?>">Next <i class="icon-arrow-right"></i></a></li><?php
    } ?>
    </ul><?php
    if( $jump_to ){ ?>
 ...
 * @return string
 */
function pagination_custom($totalposts,$p,$lpm1,$prev,$next,$page_name){
    $adjacents = 3;
    if($totalposts > 0)
    {
        $pagination = '<center><div class="tablenav"><div class="tablenav-pages" style="margin: 1em 0">';
        //previous button
        if ($p > 1)
        ...
        if ($totalposts < 7 + ($adjacents * 2)){
            for ($counter = 1; $counter <= $totalposts; $counter++){
                if ($counter == $p)
                    ...
                    $pagination.= " <a href=\"?$page_name&pagenum=$counter\">$counter</a> ";}
            ...
            if($p < 1 + ($adjacents * 2)){
                ...
                $pagination.= " ... ";
                $pagination.= " <a href=\"?$page_name&pagenum=$lpm1\">$lpm1</a> ";
                $pagination.= " <a href=\"?$page_name&pagenum=$totalposts\">$totalposts</a> ";
            }
            //in middle; hide some front and some back
            elseif($totalposts - ($adjacents * 2) > $p && $p > ($adjacents * 2)){
                $pagination.= " <a href=\"?$page_name&pagenum=1\">1</a> ";
                $pagination.= " <a href=\"?$page_name&pagenum=2\">2</a> ";
                $pagination.= " ... ";
                ...
                $pagination.= " <a href=\"?$page_name&pagenum=\">1</a> ";
                $pagination.= " <a href=\"?$page_name&pagenum=2\">2</a> ";
                $pagination.= " ... ";
        ...
        if ($p < $counter - 1)
            $pagination.= " <a href=\"?$page_name&pagenum=$next\">Next >></a>";
        else
            $pagination.= " <span class=\"disabled\">Next >></span>";


        $pagination.= "</div></div></center>\n";
    }
    return $pagination;
}
